"use client";

import type { HTMLAttributes, ReactNode } from "react";

export const liquidGlassClassName =
  "bg-transparent border border-[var(--liquid-glass-border)] shadow-[0_18px_48px_rgba(9,9,11,0.12)]";

export const liquidGlassFieldClassName =
  "bg-transparent h-12 w-full rounded-2xl border border-[var(--liquid-glass-border)] px-4 text-left text-sm text-zinc-900 dark:text-zinc-100 outline-none transition placeholder:text-zinc-500 dark:placeholder:text-zinc-400 focus:border-zinc-900/40 dark:focus:border-zinc-100/40";

export const liquidGlassMenuClassName =
  "bg-[var(--liquid-glass-surface)] overflow-hidden rounded-2xl border border-[var(--liquid-glass-border)] shadow-[0_16px_40px_rgba(9,9,11,0.12)]";

type LiquidGlassPanelProps = HTMLAttributes<HTMLDivElement> & {
  children: ReactNode;
};

export function LiquidGlassDefs() {
  return (
    <svg
      aria-hidden="true"
      focusable="false"
      width="0"
      height="0"
      className="pointer-events-none absolute opacity-0"
    >
      <filter id="displacementFilter">
        <feImage
          href="data:image/png;base64,"
          preserveAspectRatio="none"
          result="turbulence"
        />
        <feDisplacementMap
          in="SourceGraphic"
          in2="turbulence"
          scale="600"
          xChannelSelector="R"
          yChannelSelector="G"
        />
      </filter>
    </svg>
  );
}

export function LiquidGlassPanel({
  children,
  className = "",
  ...props
}: LiquidGlassPanelProps) {
  return (
    <div
      className={`rounded-4xl px-3 py-3 ${liquidGlassClassName} ${className}`}
      {...props}
    >
      {children}
    </div>
  );
}
